// supabase/functions/list-quests/index.ts
// -----------------------------------------------------------------------------
//  list-quests – Edge Function  (Deno Deploy + supabase-js v2)
//  • Guarantees the “First-Flame” quest exists & the caller is a participant
//  • Returns every quest visible via RLS, wrapped in { data, serverTimestamp }
//  • CORS helpers, logging, and Sentry plumbing are centralised in ../_shared
// 2025-05-13 – adds response wrapper to match React-Query hook contract
// -----------------------------------------------------------------------------
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    /*── Wrapper that the React-Query hook expects ────────────────*/
    const responseBody = {
      data           : payload,
